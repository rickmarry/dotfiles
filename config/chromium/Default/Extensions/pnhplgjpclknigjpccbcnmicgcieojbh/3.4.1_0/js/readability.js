...
Readability.prototype = {
            ...
            while (node) {
                ...
                if (stripUnlikelyCandidates) {
                    if (this.REGEXPS.unlikelyCandidates.test(matchString) &&
                        ...
                        node.tagName !== "BODY" &&
                        node.tagName !== "A" && node.parentNode.tagName !== "CODE") {
    ...
    _getClassWeight: function(e) {
        ...
        if (typeof(e.className) === 'string' && e.className !== '') {
            ...
            if (this.REGEXPS.positive.test(e.className) || (this._hasAncestorTag(e, 'figure') && /is-imageLoaded/.test(e.className)))
                weight += 25;
        }
    ...
    _clean: function(e, tag) {
        ...
        this._removeNodes(e.getElementsByTagName(tag), function(element) {
            ...
            if (isEmbed) {
                ...
                // clean for medium
                if (tag == 'img') {
                    if (!/progressiveMedia-thumbnail js-progressiveMedia-thumbnail/.test(element.className) && !/progressiveMedia-zoom js-progressiveMedia-inner/.test(element.className)) {
                        return false;
                    }
                } else if (tag == 'canvas') {
                    if (!/progressiveMedia-canvas js-progressiveMedia-canvas/.test(element.className)) {
                        return false;
                    }
                }
            }
    ...
    _cleanConditionally: function(e, tag) {
        ...
        this._removeNodes(e.getElementsByTagName(tag), function(node) {
            ...
            this.log("Cleaning Conditionally", node, tag, node.className, node.getAttribute('data-image-id'));
            ...
            if (this._getCharCount(node, ',') < 10) {
                ...
                // edit by luo
                // for medium images
                if (node.getElementsByTagName('figure').length > 0 && /section-inner/.test(node.className)) {
                    return false;
                }
                var haveToRemove =
